...
if (isset($_GET['id'])) {
	...
	if ($conn -> multi_query($sql) === TRUE) {
		...
		header('location: pay-users.php?success');
	}
	
}

$sql = "SELECT * FROM payments WHERE status='Pending' ORDER BY id ASC";
$data = mysqli_query($conn, $sql);
$index = 1;

$sql2 = "SELECT
		( SELECT COUNT(1) FROM payments WHERE status = 'Pending') AS payment_count,
