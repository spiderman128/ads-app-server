...
if (isset($_GET['id'])) {
	...
	if ($conn -> multi_query($sql) === TRUE) {
		...
		header('location: pay-recharge.php?success');
	}
	
}

$sql = "SELECT * FROM payments WHERE status='Pending' AND method = 'Recharge' ORDER BY id ASC";
$data = mysqli_query($conn, $sql);
$index = 1;

$sql2 = "SELECT
		( SELECT COUNT(1) FROM payments WHERE status = 'Pending' AND method = 'Recharge') AS payment_count,
